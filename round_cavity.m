...
r3 = r2*1.5;   % simulation area radius
...
% fixed vertices
fv = [ ptoncir( 0, 0, r1, 8 ) ; ptoncir( 0, 0, r2, 2*pi*r2/(h0*4) ) ];

% 2d crossection mesh
[ xr, xtri ] = distmesh( fr3, fh, h0, [-r3,-r3;r3,r3], fv, [] );
...
% Outer boundary
fout = @(p) max( -min(-(-h/2)+p(:,3),(h/2)-p(:,3)), fr3(p) );
